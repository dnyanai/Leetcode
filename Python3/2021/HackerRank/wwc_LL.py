...
class LinkedList:
    ...
    def remove(self, val):
        ...
        if self.head.data == val:  # If the node to remove is the head
            ...
            current = self.head
            while current.next is not None and current.data != val:
                previous = current
                current = current.next
...
if __name__ == "__main__":
    ...
    #my_list.display()
"""
    print("Removing the first node of linked list:")
    my_list.remove(3)
    my_list.display()
    print("Removing a node of the linked list:")
    my_list.remove(2)
    my_list.display()
"""
